function update_page() {
	...
	window.location = 'http://newa.nrcc.cornell.edu/newaTester/'+document.stationLister.pest.value;
}

function update_help() {
	var params = {type: 'grape_disease'};
	$('select[name=pest]').each(function () { params[this.name] = this.value; });
	$.get('http://newa.nrcc.cornell.edu/newaTester/process_help',params,function(data) { $('#third').html(data); });
	return false;
  }
...
function getberrymoth(selopt) {
	...
	$.get('http://newa.nrcc.cornell.edu/newaTester/process_input',params,function(data) {
