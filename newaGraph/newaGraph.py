...
def run_apple_scab_plots (stn,end_date_dt,greentip,output):
	try:
		...
		# obtain daily data
		if not daily_data:
			db_dt = start_date_dt + DateTime.RelativeDate(days=-7)					
			daily_data, station_name = newaCommon.Base().get_daily (stn, db_dt, end_date_dt)
		smry_dict['station_name'] = station_name
		
		# format for plot routine		
		obs_dict, smry_dict, forecast_data = tp_for_grf(stn, daily_data, smry_dict, start_date_dt, end_date_dt)
			
		# calculate base 0C degree days for ascospore maturity
		if greentip:
			if forecast_data:
				daily_data = daily_data + forecast_data		# merge obs and forecast data
				end_fcst = forecast_data[-1][0]
				end_fcst_dt = DateTime.DateTime(end_fcst[0],end_fcst[1],end_fcst[2],23)
			else:
